...
## Track P: a learned stop under honest expansion cost, executed

Added 2026-09-04. Screening splits only; no holdout was generated, read, or
opened; a screening ablation on new v3 data, not an arm of the frozen READ v1
experiment. Read in `experiments/track_p_v1/track_p_screening_report.md`.

**Order of evidence.** The ladder JSON and its gates were committed in `f0a104f`
before any model existed; K = 1 follows from the rule in that script's docstring.
The preregistration was committed as `23e78c3` and amended as
`b4bd881911dd2b3d722049bc2b2c6830814fed00` at 2026-09-04T03:57:18+10:00, at
roughly 250 of 8,000 updates of a first launch whose six run directories were
deleted before any checkpoint was evaluated. Every one of the six surviving
`probe.json` files carries `git_head` `b4bd881911dd` — the amendment commit
itself — and a `started_at` after it, verified mechanically (6/6 descend). The
launcher refuses to start unless HEAD descends from the preregistration commit.

  - `probe-TRAVP-deg6_len8_v4_rep-k1-s1729` started 2026-09-03T17:57:34+00:00
  - `probe-TRAVP-deg3_len6_v8-k1-s1729` started 2026-09-03T17:57:54+00:00
  - `probe-TRAVP-deg6_len8_v4_rep-k1-s2718` started 2026-09-03T17:58:15+00:00
  - `probe-TRAVP-deg3_len6_v8-k1-s2718` started 2026-09-03T17:58:35+00:00
  - `probe-TRAVP-deg6_len8_v4_rep-k1-s3141` started 2026-09-03T17:58:55+00:00
  - `probe-TRAVP-deg3_len6_v8-k1-s3141` started 2026-09-03T23:22:42+00:00 — refused
    by the launcher's GPU-memory gate (724 MiB free against a 2,000 MiB floor) and
    launched when the other five had finished; same code, same config.

**Execution.** Six runs of 8,000 updates (cuda, bf16, MPS), `model_v3` at
10,257,896 parameters — identical to Track O's, asserted by the runner — on
`deg6_len8_v4_rep-k1` and `deg3_len6_v8-k1` at γ = 0.4, no budget, K = 1, seeds
1729/2718/3141, microbatch 128 × 1, on-policy supervision throughout. All six
completed. **Stderr is not empty:** three runs (`deg3 s1729`, `deg3 s2718`,
`deg6 s2718`) each logged one `CUDACachingAllocator` OOM *warning* between
03:59 and 04:02 local, two to five minutes after launch while five runs were
allocating concurrently (the sixth was gate-refused for the same reason). The
allocator frees its cache and retries on that path; a failed retry raises and
ends the run, and all three ran to 8,000 with continuous loss curves and nineteen
screening checkpoints each. It is a memory-pressure event, not a numerical one,
and is recorded rather than described as clean. No other stderr content.

**Data.** `private/track-p-v1/data/`, generated by `read_run_v3_write_splits.py`
at 32 workers: hard cell 14,380 / 20,000 train and 2,916 / 4,000 screen (uniqueness
rejection 0.281 / 0.271), easy cell 20,000 / 20,000 and 4,000 / 4,000; measured γ
0.3981 / 0.3954 / 0.4011 / 0.4005 against 0.4 requested. **Contamination check:**
train and screen share zero episode ids, zero paired-world ids and zero identical
visible payloads on both cells.

**Observed reading, hard cell, per seed.** Registers both targets 0.973 / 0.959 /
0.951 (Wilson LB 0.967 / 0.952 / 0.943) at 5.71 / 5.72 / 5.62 expansions against
`stop_aware`'s 0.854 at 5.95 and the oracle's 4.74; overhead after registration
0.00 on every run; early stops 0.028 / 0.041 / 0.050. Bands **A / A / C** — the
third at 0.943 against the 0.95 bound, a split the outcome table did not
anticipate. Coverage disagreements between the two independent implementations:
0 / 0 / 1 of 2,179 per seed on the hard cell, 0 / 0 / 0 (probability rule) on the
easy cell. Proof-validity 0.117 / 0.128 / 0.095; returned-set precision
0.810 / 0.805 / 0.834, recall 0.769 / 0.767 / 0.713.

**Decomposition** (all 2,899 gated episodes, oracle stop held fixed): trained
ordering 5.814 / 5.819 / 5.743 expansions to register both targets, against
similarity-following 6.254 and the oracle 4.736; the stop then saves
1.97 / 1.95 / 2.03 against exhaustion's 7.799. The stop head's registration on
abstain versus non-abstain episodes: 0.972 / 0.973, 0.964 / 0.959, 0.961 / 0.951.

**Stated limitations, recorded with the result** (report §7): the preregistered
cost rule counts ties as wins and reads 0.62 against an oracle the model never
once beats; the answer head hard-codes the constant 2; the generator's promoted
edge signal separates target from distractor routes on 24.8 % of episodes with a
simulated ceiling of 0.47; the selective-return evaluation ran on non-abstain
episodes, on which a zero-parameter mask-agreement rule scores 0.909–0.949 and
0.000 on abstain episodes — the model's own rule is flat across that split
(e.g. 0.116 / 0.110) and did not exploit it; amendment 1 was made mid-run at the
point of least data; content features carry no ordering information by
construction.

**Suite.** Full canonical suite at the working tree that produced the report:
`uv run pytest -q` → **614 passed in 755.68s**. `ruff check`, `ruff format --check` and
`uv lock --check` clean. The frozen v1 contracts pass untouched; `model_v2`,
`generator_v2` and `generator_v3` are unchanged and Track O's golden fixture
still pins Track O's walk.
